...
class TestTileMath:
    ...
    def test_deg2tile_felsted(self):
        """Felsted at zoom 17 should land in a specific x/y range."""
        tx, ty = _deg2tile(CENTER_LAT, CENTER_LON, zoom=17)
        assert 65685 < tx < 65705
        assert 43370 < ty < 43390
...
class TestCanvasPixelMapping:
    """Tests for _gps_to_canvas_px using the actual world tile range."""

    _ZOOM  = 17
    _TX_MIN, _TY_MIN = 65689, 43372
    _TX_MAX, _TY_MAX = 65700, 43383
    _NX = _TX_MAX - _TX_MIN + 1   # 12
    _NY = _TY_MAX - _TY_MIN + 1   # 12
    _CW = _NX * 256
    _CH = _NY * 256
